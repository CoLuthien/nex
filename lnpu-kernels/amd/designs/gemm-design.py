...
# SPDX-FileCopyrightText: Copyright (C) 2026 the nex authors
# SPDX-License-Identifier: Apache-2.0
#
# IRON's gemm design, with one fix. Everything else is upstream, byte for byte, so that a diff
# against iron/operators/gemm/design.py shows exactly what we changed and nothing else.
#
# THE FIX (search for "nex:"):
#
# A worker waits on its runtime barrier with Acquire, which on this hardware means "wait until
# the lock reads this" and not "take it". The runtime sequence sets the lock to 1 to release the
# worker, and nothing ever puts it back -- while every other lock in the design is an
# AcquireGreaterEqual paired with a Release. So the barrier holds a worker exactly once, the
# first time it goes round.
#
# What that costs only shows when a second shape runs on the same array. The worker finishes,
# loops round its while(true), passes the barrier again at once because the lock still reads 1,
# and loads its runtime parameters before the host has written the next ones. It then blocks on
# its input fifo already committed to the previous run's trip counts.
#
# Measured on a Strix NPU: after a run at K=1536, a run at K=2048 returns what summing only the
# first 24 of its 32 tiles gives, to a relative error of 0.0087 -- the same error a correct run
# has. The eight tiles it never consumed stay in the fifos and every run after that is
# misaligned, permanently. A change in N alone survives it, because the worker's next pass makes
# up the tile it was short of and the operand counts still balance. A change in K cannot: the
# count that went stale is the one that says how much to consume.
#
# AcquireGreaterEqual subtracts what it takes, so one set_lock releases one pass, and since the
# runtime sequence writes the parameters before it sets the lock they are current at every point
# the worker can be past the barrier.
#
# The instruction stream is unaffected -- this is entirely inside the core program -- so the
# emitter's reference streams still match byte for byte and only the xclbin differs.
...
from aie.dialects.aie import LockAction, use_lock  # nex: for the barrier fix
...
def my_matmul(
    ...
    def core_fn(
        ...
        # nex: taken, not merely observed -- see the note at the top of this file.
        use_lock(barrier.worker_locks[-1], LockAction.AcquireGreaterEqual, value=1)
