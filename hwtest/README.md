...
## 알려진 한계: IRON 스톡 GEMM 은 한 컨텍스트에서 K 를 못 바꾼다

`hw_context` 하나에 shape 를 연달아 태우면, **K 가 직전 실행과 달라지는 첫 실행이 직전 K 로
계산한다.** 그 어긋남이 데이터를 좌초시키고 어레이는 회복되지 않는다.

수치가 그대로 말한다. K=1536 다음에 온 K=2048 의 1 회차를 "2048 중 앞 24 타일만 더한 값" 과
맞춰 보면 **상대오차 0.0087** — 순수 누산 오차와 같은 크기다. 회전도 없다. 즉 코어는 데이터는
제대로 짝지었고 횟수만 이전 것을 썼다.

| | 1 회차 | 2 회차 | 3 회차 |
|---|---:|---:|---:|
| K1536 N1536 (첫 shape) | 0.00887 | 0.00887 | 0.00887 |
| K1536 N2048 (N 만 바뀜) | 0.00888 | 0.00888 | 0.00888 |
| K2048 N1536 (K 바뀜) | 0.871 | 1.227 | 1.227 |
| K3072 N1536 (K 바뀜) | 1.246 | 1.415 | 1.415 |

2 회차부터는 값이 **완전히 고정**된다 — 경쟁이 아니라 고착된 어긋남이다. A 와 B 가 각자 몇
타일씩 밀린 모델로도 안 맞아서 (최적 오차 0.32 이상), 코어 행·컬럼마다 다르게 밀린 것으로
보인다. 더 파도 할 일이 바뀌지 않아 여기서 멈췄다.

**알아낸 것을 정리하면:**

- **`--isolate` 로 shape 마다 컨텍스트를 주면 넷 다 통과한다** — 구운 K 가 아닌 K 도 포함해서.
  어레이 자체는 K 를 가리지 않는다. 핸드셰이크가 가릴 뿐이다.
- **`--drain` 은 타임아웃만 없앤다.** state 8 이 사라지고 반복이 정상 속도로 돌지만 **틀린 값은
  한 비트도 안 바뀐다.** 좌초된 데이터는 shim 이 아니라 memtile 과 코어 사이에 있다.
- **N 은 바뀌어도 된다.** tile_steps 만 달라진 경우는 멀쩡하다.
- 맞을 때의 성능은 1500~2100 GFLOP/s, GEMM 하나에 380~620 us.

IRON 의 코어는 `while(true) { barrier.wait_for_value(1); rtp 읽기; 계산 }` 이고 배리어는 소비되는
`Acquire` 라, 규약대로면 코어는 실행 사이에 붙잡혀 있어야 하고 새 RTP 를 읽어야 한다. 실제로는
아니다. **이건 스톡 설계의 핸드셰이크 문제지 NPU 의 한계가 아니다** — FastFlowLM 은 `mm.xclbin`
하나로 모델의 모든 K 를 돌린다. 자기 커널을 쓰기 때문이다.
