...
## 고쳐진 것: 한 컨텍스트에서 K 바꾸기

한동안 `hw_context` 하나에 shape 를 연달아 태우면 **K 가 직전 실행과 달라지는 첫 실행이 직전
K 로 계산했다.** 그 어긋남이 데이터를 좌초시키고 어레이는 회복되지 않았다.

원인은 IRON gemm 설계의 워커 배리어였다 — `Acquire` 는 lock 을 가져가지 않고, 아무도
되돌리지 않아서, 배리어가 워커를 맨 처음 한 번만 붙잡았다. 자세한 것과 고친 내용은
`lnpu-kernels/amd/README.md` 의 `designs/` 절에 있다.

고친 xclbin 으로 **컨텍스트 하나에서 네 shape 가 모두 정확하고, 세 회차가 전부 같은 값이다.**

| shape | 1 회차 | 2 회차 | 3 회차 |
|---|---:|---:|---:|
| K1536 N1536 | 0.008869 | 0.008869 | 0.008869 |
| K1536 N2048 | 0.008881 | 0.008881 | 0.008881 |
| K2048 N1536 | 0.009504 | 0.009504 | 0.009504 |
| K3072 N1536 | 0.01057 | 0.01057 | 0.01057 |

정상 실행의 상대 L2 는 K 를 따라 **K 의 1/4 승**으로 자란다. bf16 누산이 남기는 것이고
틀린 것이 아니라 덜 정밀한 것이다. 허용치 2% 는 그래서 최대 측정치의 두 배로 잡았다.

### `--drain` 은 필요 없었다

드레인은 이 버그를 쫓는 과정에서 만든 것이다. 타임아웃(state 8)을 없앴지만 틀린 값은 한 비트도
바꾸지 못했다 — 좌초된 데이터가 shim 이 아니라 memtile 과 코어 사이에 있었기 때문이다.
그 좌초 자체가 배리어 버그의 결과였다. 고친 뒤 **드레인 없이 7/7** 이고, 그래야 GEMM 도
aiecc 정답지 대조를 유지한다. 진단 도구로 남겨 두되 기본은 꺼짐이다.

## 검증 결과 (NPU Strix, 2026-08-23)

**7/7 통과** — 옵션 없이 `nex-hwtest <아티팩트>`. rmsnorm 설계 3 개와 gemm shape 4 개가
모두 aiecc 스트림과 바이트 일치하고 수치도 맞는다. gemm 네 shape 는 hw_context 하나를
공유하며, 세 회차가 전부 같은 값이다.

| | 상대 L2 | 100 회 평균 |
|---|---:|---:|
| gemm M128 K1536 N1536 | 0.008869 | 410 us (1475 GFLOP/s) |
| gemm M128 K1536 N2048 | 0.008881 | 374 us (2151 GFLOP/s) |
| gemm M128 K2048 N1536 | 0.009504 | 414 us (1945 GFLOP/s) |
| gemm M128 K3072 N1536 | 0.01057 | 616 us (1960 GFLOP/s) |
| rmsnorm (3 개) | 0 / 0 / 0.004174 | 150~164 us |

rmsnorm 3/3 통과. 무가중치 두 케이스는 CPU 레퍼런스와 **비트 단위 일치**, 가중치 케이스는
bf16 곱셈 반올림으로 상대오차 0.0115 (허용치 4% 안). 141~162 µs.

GEMM 은 shape 마다 컨텍스트를 주면 (`--isolate`) 4/4 통과한다. 상대 L2 는 K 를 따라
0.00887 / 0.00888 / 0.00950 / 0.01057 로 자란다 (K 의 1/4 승). 컨텍스트를 공유하면 K 가 바뀌는
순간부터 깨진다 — 위 "알려진 한계" 참고.
